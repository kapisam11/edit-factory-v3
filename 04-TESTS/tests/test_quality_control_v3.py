...
from ai_video_factory import quality_control_v3
...
def test_black_frame_parser_accepts_ffmpeg_colon_format(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(quality_control_v3, "run_ffmpeg", fake_run_ffmpeg)
    video = tmp_path / "video.mp4"
    video.write_bytes(b"placeholder")
    result = quality_control_v3.check_black_frames(str(video))
...
def test_frozen_frame_parser_accepts_ffmpeg_colon_format(monkeypatch, tmp_path):
    ...
    video = tmp_path / "video.mp4"
    video.write_bytes(b"placeholder")
    result = quality_control_v3.check_frozen_frames(str(video))
