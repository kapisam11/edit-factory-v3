"""AI Video Factory — NLE Export v3.
...
def export_fcpxml(
    ...
    ET.SubElement(resources, "format", {
        "id": "r1", "name": "FFVideoFormat1080x1920p30",
        "frameDuration": "1/30s", "width": "1080", "height": "1920",
    })
    asset_ids = {}
    for i, clip_path in enumerate(clips):
        ...
        ET.SubElement(resources, "asset", {
            "id": aid, "name": os.path.basename(clip_path),
            "src": f"file://{os.path.abspath(clip_path)}",
            "hasVideo": "1", "hasAudio": "1", "duration": "1s",
        })
    ...
    for i, clip_path in enumerate(clips):
        ...
        clip_el = ET.SubElement(spine, "clip", {
            "name": os.path.basename(clip_path), "offset": f"{current_time}s",
            "duration": f"{duration}s", "start": "0s",
        })
        video = ET.SubElement(clip_el, "video")
        ET.SubElement(video, "asset-clip", {"ref": asset_ref, "duration": f"{duration}s"})
...
def export_premiere_xml(
    ...
    for i, clip_path in enumerate(clips):
        ...
        clip_el = ET.SubElement(track, "ClipItem", {
            "Id": f"clip_{i}", "Name": os.path.basename(clip_path),
            "Start": str(current_time), "End": str(current_time + duration),
            "In": "0", "Out": str(duration),
        })
...
def export_capcut_json(
    ...
    for i, clip_path in enumerate(clips):
        ...
        if plan and i < len(plan.get("edit_plan", [])):
            duration = plan["edit_plan"][i].get("duration", 2.0)
        tracks.append({
            "id": f"clip_{i}", "type": "video", "source": os.path.abspath(clip_path),
            "start_time": current_time, "duration": duration,
            "effects": plan["edit_plan"][i].get("label", "") if plan and i < len(plan.get("edit_plan", [])) else "",
        })
        current_time += duration
    project = {
        "project_name": "AI Video Factory Export", "created_at": datetime.now().isoformat(),
...
def export_all_nle_formats(package_dir: str, clips: List[str], plan: Optional[Dict] = None) -> Dict[str, str]:
