...
Offline-safe and provider-agnostic: measured outcomes become bounded,
explainable recommendations for future V3 blueprints.
"""
...
class ChannelPerformanceModel:
    ...
    def edit_type_lift(self) -> Dict[str, float]:
        ...
        overall = mean(self._engagement(x) for x in self.observations)
    ...
    def recommend(self, *, current_edit_type: str | None = None) -> LearningRecommendation:
        if self.sample_size < 3:
            return LearningRecommendation(0.0, "collect-data", ("Keep the V3 baseline strategy; collect at least 3 measured outcomes before adapting.",), self.sample_size, "Insufficient evidence for a stable channel-specific recommendation.")
