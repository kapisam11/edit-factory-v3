...
def _run(command: Sequence[str], *, timeout: int | None = None, capture_output: bool = True, text: bool = True) -> subprocess.CompletedProcess:
    try:
        return subprocess.run(
            list(command), check=True, capture_output=capture_output, text=text,
...
def probe_media(path: str) -> Dict[str, Any]:
    ...
    return {
        "duration": duration, "size": int(payload.get("format", {}).get("size") or os.path.getsize(path)),
        "width": width, "height": height, "fps": video.get("r_frame_rate") or video.get("avg_frame_rate") or "0/0",
        "has_audio": audio is not None, "video_codec": video.get("codec_name"),
        "audio_codec": audio.get("codec_name") if audio else None, "format_name": payload.get("format", {}).get("format_name"),
    }


def normalize_duration(input_path: str, target_path: str, target_seconds: float) -> str:
    if not math.isfinite(float(target_seconds)) or not 0.25 <= float(target_seconds) <= 180.0:
...
def enforce_retention_events(input_path: str, output_path: str, retention_events: Sequence[Mapping[str, Any]]) -> str:
    ...
    try:
        _run(["ffmpeg", "-hide_banner", "-loglevel", "error", "-y", "-i", input_path, "-vf", ",".join(filters), "-map", "0:v:0", "-map", "0:a:0?", "-c:v", "libx264", "-preset", "medium", "-crf", "18", "-c:a", "aac", "-movflags", "+faststart", temp_path], timeout=_timeout("AIVF_FFMPEG_TIMEOUT_SECONDS", 3600))
...
def _sample_visual_changes(path: str, sample_hz: float = 10.0) -> Dict[str, Any]:
    ...
    command = ["ffmpeg", "-hide_banner", "-loglevel", "error", "-i", path, "-t", f"{sample_duration:.3f}", "-vf", f"fps={sample_hz:g},scale=160:90,format=gray", "-f", "rawvideo", "pipe:1"]
    result = _run(command, timeout=_timeout("AIVF_FFMPEG_TIMEOUT_SECONDS", 3600), capture_output=True, text=False)
    frame_size = 160 * 90; raw = result.stdout if isinstance(result.stdout, (bytes, bytearray)) else b""
