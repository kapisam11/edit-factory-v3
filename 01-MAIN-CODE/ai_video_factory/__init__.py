...
from .performance_learning import ChannelPerformanceModel, LearningRecommendation, PerformanceObservation

class VideoDirector(_LegacyVideoDirector):
    """Public director API with the modern production pipeline as the footage path."""
    def produce(self, topic, raw_video=None, use_groq=False, groq_key=None, target_seconds=45.0, skip_qc=False):
        if not raw_video:
            return super().produce(topic, raw_video=None, use_groq=use_groq, groq_key=groq_key, target_seconds=target_seconds, skip_qc=skip_qc)
        ...
        result = run_production_pipeline(raw_video, topic, package_dir, target_seconds=target_seconds, research_summary=research_summary, model_key=self.model_key, skip_qc=skip_qc)
        self._last_production_result = result
        self.creative_brief = research_summary or {}
        if result.errors: raise RuntimeError("Production pipeline failed: " + "; ".join(result.errors))
...
__version__ = "3.0.0"
__all__ = ["VideoDirector", "create_package", "compose_short_from_video", "run_production_pipeline", "run_v3_pipeline", "create_v3_blueprint", "validate_blueprint", "V3Blueprint", "V3Config", "EditType", "Scene", "TimelineSegment", "EditTimeline", "ChannelPerformanceModel", "LearningRecommendation", "PerformanceObservation"]
