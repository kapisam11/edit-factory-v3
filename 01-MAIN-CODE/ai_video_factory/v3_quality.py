...
def _run(command: Sequence[str], *, timeout: int | None = None, capture_output: bool = True) -> subprocess.CompletedProcess[str]:
    try:
        return subprocess.run(
            list(command), check=True, capture_output=capture_output, text=True,
...
def probe_media(path: str) -> Dict[str, Any]:
    ...
    result = _run([
        "ffprobe", "-v", "error", "-show_entries", "format=duration,size,format_name,bit_rate",
    ...
    if duration <= 0 or width <= 0 or height <= 0:
        raise RenderContractError("rendered artifact has invalid duration or dimensions")
    return {
        "duration": duration,
...
def normalize_duration(input_path: str, target_path: str, target_seconds: float) -> str:
    if not math.isfinite(float(target_seconds)) or not 0.25 <= float(target_seconds) <= 300.0:
        raise RenderContractError("target duration must be between 0.25 and 300 seconds")
    info = probe_media(input_path)
    current = float(info["duration"]); delta = float(target_seconds) - current
    if abs(delta) <= 0.05:
    ...
    fd, temp_path = tempfile.mkstemp(suffix=".mp4", dir=os.path.dirname(target_path) or "."); os.close(fd)
    try:
        command = ["ffmpeg", "-hide_banner", "-loglevel", "error", "-y", "-i", input_path]
        if delta > 0:
            command += ["-vf", f"tpad=stop_mode=clone:stop_duration={delta:.3f}", "-af", "apad"]
        command += [
            "-t", f"{float(target_seconds):.3f}", "-map", "0:v:0", "-map", "0:a:0?", "-c:v", "libx264",
            "-preset", "medium", "-crf", "18", "-c:a", "aac", "-movflags", "+faststart", temp_path,
        ]
        ...
        if abs(normalized["duration"] - float(target_seconds)) > 0.08:
            raise RenderContractError(f"duration normalization failed: {normalized['duration']:.3f}s != {target_seconds:.3f}s")
...
def enforce_retention_events(input_path: str, output_path: str, retention_events: Sequence[Mapping[str, Any]]) -> str:
    ...
    info = probe_media(input_path)
    kind_settings = {"zoom": (1.04, 0.045), "text": (1.02, 0.030), "motion": (1.05, 0.035), "angle": (1.06, 0.025), "clip": (1.08, 0.030), "beat drop": (1.14, 0.020)}
    filters = []
    ...
    try:
        _run([
            "ffmpeg", "-hide_banner", "-loglevel", "error", "-y", "-i", input_path, "-vf", ",".join(filters),
            "-map", "0:v:0", "-map", "0:a:0?", "-c:v", "libx264", "-preset", "medium", "-crf", "18",
            "-c:a", "aac", "-movflags", "+faststart", temp_path,
        ], timeout=_timeout("AIVF_FFMPEG_TIMEOUT_SECONDS", 3600))
        probe_media(temp_path); os.replace(temp_path, output_path); return output_path
    except BaseException as exc:
        try: os.unlink(temp_path)
        except OSError: pass
        if isinstance(exc, RenderContractError): raise
...
def _sample_visual_changes(path: str, sample_hz: float = 10.0) -> Dict[str, Any]:
    if not math.isfinite(sample_hz) or not 0.0 < sample_hz <= 30.0:
        raise ValueError("sample_hz must be > 0 and <= 30")
    info = probe_media(path)
    # V3 assets are short-form; cap the raw-frame QC stream to bound memory/disk usage.
    sample_duration = min(float(info["duration"]), 180.0)
    command = [
        "ffmpeg", "-hide_banner", "-loglevel", "error", "-i", path, "-t", f"{sample_duration:.3f}",
        "-vf", f"fps={sample_hz:g},scale=160:90,format=gray", "-f", "rawvideo", "pipe:1",
    ]
    result = _run(command, timeout=_timeout("AIVF_FFMPEG_TIMEOUT_SECONDS", 3600), capture_output=True)
    frame_size = 160 * 90; raw = result.stdout.encode("latin1", errors="ignore") if isinstance(result.stdout, str) else result.stdout
    frame_count = len(raw) // frame_size
    if frame_count < 2: return {"frames": frame_count, "change_events": [], "max_gap": None}
    deltas = []
    for index in range(1, frame_count):
        previous = raw[(index - 1) * frame_size:index * frame_size]; current = raw[index * frame_size:(index + 1) * frame_size]
        deltas.append(sum(abs(a - b) for a, b in zip(previous, current)) / frame_size)
    ordered = sorted(deltas); threshold = max(6.0, ordered[min(len(ordered) - 1, int(len(ordered) * 0.65))] * 0.70)
    change_events = [round((index + 1) / sample_hz, 3) for index, delta in enumerate(deltas) if delta >= threshold]
    max_gap = None
    if change_events:
...
def strict_render_check(path: str, *, target_seconds: float, platform_profile: Mapping[str, Any], retention_events: Sequence[Mapping[str, Any]] = ()) -> Dict[str, Any]:
    info = probe_media(path); expected_width = int(platform_profile.get("width") or 0); expected_height = int(platform_profile.get("height") or 0)
    ...
    if not events: errors.append("retention map is empty")
        ...
        detected = visual.get("change_events", []); missing = [event for event in events if event <= 180.0 and not any(abs(event - change) <= 0.30 for change in detected)]
