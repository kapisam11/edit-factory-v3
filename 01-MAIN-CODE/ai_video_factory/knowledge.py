...
# Compatibility shim: provide the older `KnowledgeBase` API expected by callers.
class KnowledgeBase:
    """Compatibility wrapper exposing the legacy API backed by RealKnowledgeBase."""

    def __init__(self, knowledge_dir: Optional[str] = None):
        root = knowledge_dir or "knowledge_base_v2"
        self._real = RealKnowledgeBase(root_dir=root)

    def get_topic_expertise(self, topic: str) -> Dict[str, Any]:
        # Try loading a topic JSON if present, else fall back to topic keywords
        path = Path(self._real.root) / f"{topic}.json"
        if path.exists():
            try:
                ...
                return {}
        # Fallback: return topic keywords as a simple dict
        kws = self._real.topic_sim.topic_keywords.get(topic, [])
        return {"keywords": kws}

    def get_trending_techniques(self) -> List[str]:
        tpath = Path(self._real.root) / "trending.json"
        if tpath.exists():
            try:
                ...
                pass
        # Heuristic fallback: return top filters by effectiveness
        try:
            ranked = self._real.filter_tracker.rank_filters()
...
def find_persona(name: str) -> Optional[Dict[str, Any]]:
    path = ROOT / "personas.json"
    try:
        if path.exists():
            with path.open("r", encoding="utf-8") as fh:
